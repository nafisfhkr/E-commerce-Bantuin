...
class CustomerOrderDetailScreen extends StatefulWidget {
  ...
  const CustomerOrderDetailScreen({Key? key, required this.orderId})
    : super(key: key);
...
class _CustomerOrderDetailScreenState extends State<CustomerOrderDetailScreen> {
  ...
  Future<void> _processPayment(Map<String, dynamic> orderData) async {
    final PaymentService _paymentService = PaymentService();
    ...
    try {
      ...
      final num finalAmount = orderData['finalAmount'] ?? 0;
      final int amountAsInt = finalAmount.toInt();
        ...
        if (isPaid) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'Status: ${lastStatus['statusMessage'] ?? "Belum terdeteksi"}. Silakan cek beberapa saat lagi.',
                ),
  ...
  Future<void> _manualCheck(String merchantOrderId) async {
    ...
    final status = await _service.checkTransactionStatus(
      merchantOrderId: merchantOrderId,
    );
    if (mounted) Navigator.pop(context);
    ...
    if (status['statusCode'] == '00') {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Masih belum dibayar."),
  ...
  String _getStatusImage(String status) {
    switch (status) {
      case 'pending':
        return 'assets/images/status_pending.png';
      case 'accepted':
      case 'processing':
        return 'assets/images/status_processing.png';
      case 'work_done':
        return 'assets/images/status_work_done.png';
      case 'completed':
        return 'assets/images/status_completed.png';
      case 'rejected':
      case 'cancelled':
        return 'assets/images/status_rejected.png';
      default:
        return 'assets/images/status_default.png';
    }
  }
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting)
            return Center(
              child: CircularProgressIndicator(color: Color(0xFF192F65)),
            );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(
              'Waktu Pesanan',
              orderData['createdAt'] != null && orderData['providerId'] != null
                  ? '${DateTime.now().difference(orderData['createdAt'].toDate()).inMinutes} Menit'
                  : 'Belum diterima',
            ),
            const SizedBox(height: 8),
            _buildInfoRow(
  ...
  Widget _buildServiceItemCard(Map<String, dynamic> details) {
    ...
    if (details.containsKey('type') || details.containsKey('vehicleType')) {
      displayTitle = _capitalizeEachWord(
        details['service'] as String? ?? 'Servis Kendaraan',
      );
      String itemType = _capitalizeEachWord(
        details['type'] as String? ?? 'Kendaraan',
      );
      String categoryType = _capitalizeEachWord(
        details['categoryType'] as String? ?? '',
      );
      displaySubtitle = '$itemType • $categoryType';

      imagePath =
          details['itemImageUrl'] as String? ??
          'assets/images/vehicles/${(details['type'] as String? ?? '').toLowerCase()}/${(details['categoryType'] as String? ?? '').toLowerCase()}.png';
      fallbackIcon =
          (details['type'] as String? ?? '').toLowerCase() == 'mobil'
              ? Icons.directions_car
              : Icons.motorcycle;
    } else {
      displayTitle = _capitalizeEachWord(
        details['service'] as String? ?? 'Servis Elektronik',
      );
      ...
      String modelKey =
          (details['electronicModel'] as String? ??
                  details['service'] as String? ??
                  '')
              .toLowerCase();

      String typeLabel = typeLabels[typeKey] ?? _capitalizeEachWord(typeKey);
      String modelLabel =
          modelLabels[modelKey] ?? _capitalizeEachWord(modelKey);

      displaySubtitle = '$typeLabel • $modelLabel';

      imagePath =
          details['itemImageUrl'] as String? ??
          'assets/images/elektronik/layanan/perbaikan mesin.png';
                ...
                Row(
                  children: [
                    CircleAvatar(
                      ...
                      child:
                          providerPhotoUrl == null || providerPhotoUrl.isEmpty
                    ...
                    IconButton(
                      ...
                      onPressed:
                          () => Navigator.pushNamed(
                            context,
                            '/chat',
                            arguments: {'receiverUserId': providerId},
                          ),
                    ),
                    IconButton(
                      icon: Image.asset(
  ...
  Widget _buildPaymentInfoCard(Map<String, dynamic> orderData) {
    num totalAmount =
        ...
        (orderData['subtotal'] ?? 0) + (orderData['serviceFee'] ?? 0);
    return Card(
      color: Colors.white,
      elevation: 1,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.0)),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildPaymentRow(
              'Subtotal',
              'Rp. ${NumberFormat('#,###', 'id_ID').format(orderData['subtotal'] ?? 0)},00',
            ),
            SizedBox(height: 4),
            _buildPaymentRow(
              'Biaya Layanan',
              'Rp. ${NumberFormat('#,###', 'id_ID').format(orderData['serviceFee'] ?? 0)},00',
            ),
            SizedBox(height: 4),
            Divider(height: 24),
            Row(
              ...
              children: [
                ...
                Text(
                  'Rp. ${NumberFormat('#,###', 'id_ID').format(totalAmount)},00',
                  style: GoogleFonts.poppins(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: Color(0xFF192F65),
      ...
      return ElevatedButton.icon(
        ...
        onPressed:
            () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => PetaGelap(orderId: widget.orderId),
              ),
            ),
        style: ElevatedButton.styleFrom(
          backgroundColor: Color(0xFF192F65),
          foregroundColor: Colors.white,
          minimumSize: Size(double.infinity, 50),
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(12),
    ...
    if (status == 'work_done') {
      ...
      if (finalAmount != null && finalAmount > 0) {
        ...
        return ElevatedButton.icon(
          ...
          onPressed: () => _processPayment(orderData),
          style: ElevatedButton.styleFrom(
            backgroundColor: Colors.green,
            foregroundColor: Colors.white,
        ...
        return Container(
          ...
          child: Text(
            'Menunggu teknisi menetapkan harga akhir...',
